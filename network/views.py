...
def remove_like(request, post_id):
    ...
    like_count = post.post_like.count()  # Get updated like count
...
def index(request):
   ...
   #Pagination
    pageinator = Paginator(allPosts, 10)
    page_numeber = request.GET.get('page')
    posts_of_the_page = pageinator.get_page(page_numeber)
...
def following(request):
    ...
    for post in allPosts: 
     ...
     #Pagination
    pageinator = Paginator(followingPosts, 10)
    page_numeber = request.GET.get('page')
    posts_of_the_page = pageinator.get_page(page_numeber)
